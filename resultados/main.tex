...
Los resultados del test de memoria se repiten al presentar los resultados correspondientes a las tareas 
...
entre la memoria del usuario y los resultados que obtiene utilizando \mbox{\foreign{TamTam Listens}.}
...
Entre las correlaciones identificadas se observan algunas que podr\'ian resultar poco intuitivas. 
Por ejemplo,  las correlaciones positivas de la tasa de acierto del sistema ($A$) con la tasa de 
error humano ($E_2$) y con la cantidad de errores por usuario ($E_3$), que podr\'ian explicarse
con lo que se pudo observar en las sesiones de prueba. 
En las mismas se hizo notorio que los usuarios para quienes el sistema presenta una tasa de
aciertos elevada prefieren descubrir y aprender los comandos a trav\'es de la prueba y el error 
a leer m\'as detalladamente el manual de la aplicaci\'on.

As{{\'\i}} tambi\'en, la cantidad total de comandos que utilizados por el usuario ($U$) 
se relaciona positivamente con ambas medidas del error humano ($E_2$ y $E_3$) y con el 
acierto del sistema ($A$). 
En base a las observaciones, pudo notarse que ante una tasa de aciertos del sistema elevada,
los usuarios intentan descubrir y utilizar nuevos comandos.

La memoria del usuario ($M$) se relaciona negativamente con las medidas 
de error humano ($E_2$ y $E_3$) y la duraci\'on de las tareas ($T_{1+2}$ y $T_{3+4}$), lo cual 
se esperaba dadas la caracter{\'\i}sticas propias de las interfaces mediante voz del usuario como
la transitoriedad y la invisibilidad.

Ciertas correlaciones previsibles se verifican mediante los resultados. 
Los usuarios para quienes el sistema presenta una tasa de acierto superior obtienen mejores 
resultados en las tareas, seg\'un confirma la correlaci\'on entre la tasa de acierto del 
sistema ($A$) y la correctitud de la \'ultima tarea ($C$). 

As{\'\i} tambi\'en, como los errores toman tiempo, los usuarios que m\'as los cometen tardan m\'as en 
terminar las tareas. Esto se evidencia con  la correlaci\'on entre el error humano ($E_2$) y 
el tiempo para terminar las tareas ($T_{1+2}$).

\section{An\'alisis del Error Humano}
\label{sec:resultados-error-humano}

En esta secci\'on se presentan las tasas de errores de acuerdo a factores que se consideran 
importantes debido al tipo de aplicaci\'on desarrollada. 
